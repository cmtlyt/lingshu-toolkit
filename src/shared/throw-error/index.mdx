...
> author: cmtlyt
>
> update time: 2026/04/28 13:37:00
...
- **支持错误链（`cause`）**：可通过 `{ cause }` 选项传递原始错误，便于错误追踪（ES2022）
...
```ts
import { throwError, throwType, createError } from '@cmtlyt/lingshu-toolkit/shared/throw-error';

// 将底层错误作为 cause 传递，便于错误链追踪
try {
  await fetchData();
} catch (originalError) {
  throwError('loadConfig', 'Failed to load config', { cause: originalError });
}

// 自定义错误类型 + cause
throwError('validate', 'Invalid input', RangeError, { cause: originalError });

// throwType + cause
throwType('parse', 'Expected JSON string', { cause: originalError });

// createError + cause
const error = createError('task', 'Task failed', { cause: originalError });
...
### throwError(fnName, message, ErrorClass?, options?)

抛出一个错误，错误消息会自动添加包名前缀。支持两种调用形式：`throwError(fnName, message, ErrorClass?, options?)` 或 `throwError(fnName, message, options?)`（省略 `ErrorClass` 默认为 `Error`）。
...
- **options?**: `{ cause?: unknown }`
  - 可选配置；`cause` 用于携带原始错误，便于错误链追踪（ES2022 `Error.cause`）
...
throwError('myFunction', 'Task failed', { cause: originalError });
// 抛出: Error: ...; error.cause === originalError

throwError('myFunction', 'Out of range', RangeError, { cause: originalError });
// 抛出: RangeError: ...; error.cause === originalError
```

### throwType(fnName, message, options?)
...
throwType('myFunction', 'Invalid JSON', { cause: parseError });
// 抛出: TypeError: ...; error.cause === parseError
```

### createError(fnName, message, ErrorClass?, options?)

创建一个错误对象，错误消息会自动添加包名前缀，但不会抛出。支持两种调用形式：`createError(fnName, message, ErrorClass?, options?)` 或 `createError(fnName, message, options?)`。
...
const errWithCause = createError('myFunction', 'Task failed', { cause: originalError });
console.log(errWithCause.cause); // → originalError
