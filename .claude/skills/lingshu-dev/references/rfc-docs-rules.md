...
7. **RFC 评审通过后才允许创建实施清单**：RFC 状态为 `draft` 时禁止创建 `IMPLEMENTATION.md`，必须等用户明确评审通过（状态变为 `accepted`）后才能创建
8. **RFC 评审通过后自动创建实施清单**：当用户确认 RFC 评审通过时，立即将 RFC 状态更新为 `accepted` 并自动创建对应的 `IMPLEMENTATION.md`，无需用户额外指示
